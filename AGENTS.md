...
- Team management uses the centralized permission matrix in `src/lib/customer-auth/permissions.ts` and the audited `/api/team` service. Owners manage administrators; administrators manage members and reviewers; neither may change their own role or the owner. Pending invitations reserve subscription seats and acceptance rechecks the exact verified email and capacity. Do not re-enable parallel Better Auth team-mutation routes that bypass these controls. See `docs/customer-auth.md`.
